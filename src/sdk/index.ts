...
export default class Sdk {
  ...
  flattenAndRemoveDuplicates = (data: any) => {
    const seenPaths = new Set();  // 用于存储已出现过的路径
    const result: any = [];  // 用于存储最终的去重拍平后的数据

    // 递归函数，用于扁平化数据并去重
    function recurse(item: any) {
      // 如果该路径已经存在，跳过该项
      if (seenPaths.has(item.insId)) {
        return;
      }

      // 将当前路径添加到结果数组
      result.push(item);
      seenPaths.add(item.insId);  // 将路径添加到 Set 中，确保去重

      // 如果有子项，则递归处理子项
      if (item.children && Array.isArray(item.children)) {
        item.children.forEach((child: any) => recurse(child));  // 递归处理每个子项
      }
    }

    // 遍历数据，开始递归处理每一项
    data.forEach((item: any) => recurse(item));
          ...
          try {
            for (let attachmentData of instance.attachments) {
              ...
              PrintUtils.autoUploadPrintFileToAttachment({
                ...
                insVersion: instance.basicReadInstanceInfo.insVersionUnbound,
